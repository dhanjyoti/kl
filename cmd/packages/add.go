...
var addLibCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err := addLibraries(apic, fc, cmd, args); err != nil {
			fn.PrintError(err)
			return
		}
	},
...
func addLibraries(apic apiclient.ApiClient, fc fileclient.FileClient, cmd *cobra.Command, args []string) error {
	fc, err := fileclient.New()
	if err != nil {
		return fn.NewE(err)
	}
	...
	spinner.Client.Resume()
	klConf.Libraries = append(klConf.Libraries, name)
	err = fc.WriteKLFile(*klConf)
	if err != nil {
		return functions.NewE(err)
	}
	...
	fn.Println(fmt.Sprintf("Library %s is added successfully", name))
